...
task_path = os.path.join(current_dir, "lac_model")

# 检查模型目录是否存在，如果不存在则创建
if not os.path.exists(task_path):
    os.makedirs(task_path, exist_ok=True)

# 使用Taskflow加载模型，但不指定保存路径，让PaddleNLP自动下载模型
try:
    word_segmenter = Taskflow("word_segmentation")  # 移除task_path参数，让PaddleNLP自动下载模型
